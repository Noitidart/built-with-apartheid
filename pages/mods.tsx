...
import { getMe } from '@/lib/auth.backend';
...
export const getServerSideProps = withPrismaSSR(
  async function getServerSideProps(prisma, context) {
    const me = await getMe(context, prisma);
...
function ModsPage(props: TModsPageProps) {
  return (
    <>
      <motion.h1
      ...
      >
        <Link href="/mods">Moderator Dashboard</Link>
      </motion.h1>
      ...
      >
        <Link href="/">Built with Apartheid</Link>
      </motion.h2>
      ...
      >
        ...
        {props.me && !props.me.isMod && <AccessDenied />}

        {props.me && props.me.isMod && <Dashboard me={props.me} />}
      </motion.main>
    </>
  );
}
...
type TLogoutButtonProps = Pick<TButtonProps, 'size' | 'className' | 'outlined'>;
function LogoutButton(props: TLogoutButtonProps) {
  const logoutMutation = useLogoutMutation();
...
function Dashboard(props: TDashboardProps) {
  return (
    <>
      <p className="mb-4">
        Welcome {props.me.email} to the moderator dashboard
      </p>
...
function useLoginMutation() {
  return useMutation({
    ...
    onSuccess: function reloadPageOnLoginSuccess() {
...
function useLogoutMutation() {
  return useMutation({
    ...
    onSuccess: function reloadPageOnLogoutSuccess() {
      window.location.reload();
    },
